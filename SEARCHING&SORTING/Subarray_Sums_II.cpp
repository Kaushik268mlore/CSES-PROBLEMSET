...
//     }
// };
// void solve(){        // THIS METHOD CAN BE USED TO GET CONTIGUOUS SUBSEQUENCE AS WELL..
//     int n,x;cin>>n>>x;
...
//         else{
//             int a=1,b=1;
//             while(i<n&&arr[i+1]==arr[i]){i++;a++;}
//             while(j<n&&arr[j+1]==arr[j]){j++;b++;}
//             cnt+=(a*b);
//             i++,j++;
//         }
...
void solve(){
    ...
    for(int i=0;i<n;i++){
        s+=arr[i];
        if(map.find(s-x)!=map.end())cnt+=map[s-x];
        map[s]++;
    }
    cout<<cnt<<endl;
}
